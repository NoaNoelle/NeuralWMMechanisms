...
"""
Content-Specific Reactivation Model.

Retains information by means of an STSP mechanism 
in the memory population, but the memory population is additionally reactivated on the 
basis of a 30 Hz sine wave with stimulus-specific reactivation. This is achieved by 
maintaining the representation of the original memory stimuli alive in the sensory 
population by means of a gated recurrent connection.
"""
...
def get_data(inputs_first, inputs_second, inputs_reactivate, inputs_gate, initialangles, n_trials, probelist, res):
    start = timer()
...
def plot_sim_1(sp_1,sp_2,res_1,res_2,cal_1,cal_2, mem_1, mem_2):
    ...
    figure_name = 'Sim_1/full_model/exp2_content_specific_reactivation_model/exp2_figure'
    ...
    if sim_to_run == 2:
        ...
        for subj in range(n_subj):
            ...
            for batch in range(n_batches):
                ...
                get_data(input_data_first, input_data_second, input_data_reactivate, input_data_gate, data_initialangles, minibatch_size, probelist, res)
                ...
                if store_decisions:
                    for in_batch_trial in range(minibatch_size):
                        ...
                        np.savetxt(data_path+"Performance/full_model/exp2_content_specific_reactivation_model/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe1.csv" % (anglediff, subj+1, trial),
                            sim.data[model.p_dec_first][in_batch_trial,int((1800)/res):int((2100)/res),:], delimiter=",")

                        np.savetxt(data_path+"Performance/full_model/exp2_content_specific_reactivation_model/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe2.csv" % (anglediff, subj+1, trial),
                ...
                if store_memory:
                    ...
                    if (batch+1)%(n_batches/split)==0:
                        ...
                        np.save(data_path+"Decoding/full_model/exp2_content_specific_reactivation_model/subj_%i_mem_neuron_data_first_%i.npy" % (subj+1, split_index), neuro_mem_first)
                        np.save(data_path+"Decoding/full_model/exp2_content_specific_reactivation_model/subj_%i_mem_neuron_data_second_%i.npy" % (subj+1, split_index), neuro_mem_second)
                        np.save(data_path+"Decoding/full_model/exp2_content_specific_reactivation_model/subj_%i_initial_angles_%i.npy" % (subj+1, split_index), initialangles)
