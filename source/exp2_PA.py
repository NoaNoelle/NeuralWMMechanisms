...
Retains information by means of a recurrent activity connection on 
the memory population. Memory population contains STP neurons, but the STSP mechanism has 
effectively been removed from the population by removing the STP learning rule. Neurons in the 
memory population therefore act like LIF neurons, and the recurrent connection feeds the 
activation from the memory population back to itself in order to maintain information in a
sustained neural code. 
"""
...
def get_data(inputs_first, inputs_second, inputs_reactivate, initialangles, n_trials, probelist, res):
    start = timer()
...
def plot_sim_1(sp_1,sp_2,res_1,res_2,cal_1,cal_2, mem_1, mem_2):
    ...
    figure_name = "Sim_1/full_model/exp2_persistent_activity_model/exp1_figure"
    ...
    if sim_to_run == 2:
        ...
        for subj in range(n_subj):
            ...
            for batch in range(n_batches):
                ...
                get_data(input_data_first, input_data_second, input_data_reactivate, data_initialangles, minibatch_size, probelist, res)
                ...
                if store_decisions:
                    for in_batch_trial in range(minibatch_size):
                        ...
                        np.savetxt(data_path+"Performance/full_model/exp2_persistent_activity_model/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe1.csv" % (anglediff, subj+1, trial),
                            sim.data[model.p_dec_first][in_batch_trial,int((1800)/res):int((2100)/res),:], delimiter=",")

                        np.savetxt(data_path+"Performance/full_model/exp2_persistent_activity_model/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe2.csv" % (anglediff, subj+1, trial),
                ...
                if store_memory:
                    ...
                    if (batch+1)%(n_batches/split)==0:
                        ...
                        np.save(data_path+"Decoding/full_model/exp2_persistent_activity_model/subj_%i_mem_neuron_data_first_%i.npy" % (subj+1, split_index), neuro_mem_first)
                        np.save(data_path+"Decoding/full_model/exp2_persistent_activity_model/subj_%i_mem_neuron_data_second_%i.npy" % (subj+1, split_index), neuro_mem_second)
                        np.save(data_path+"Decoding/full_model/exp2_persistent_activity_model/subj_%i_initial_angles_%i.npy" % (subj+1, split_index), initialangles)
