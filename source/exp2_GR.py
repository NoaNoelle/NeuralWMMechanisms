...
"""
Generic Reactivation Model.

Retains information by means of an STSP mechanism 
in the memory population, but the memory population is additionally reactivated
every 100 ms (10Hz rhythm) in order to evoke sustained neural activation and 
strengthen the pattern of synapti facilitation to prolong retention of WM 
information.
"""
...
def maintain_func_first(t):
    if t > 0.500 and t < 0.520:
        return np.ones(Nm)*0.02
    elif t > 0.600 and t < 0.620:
        return np.ones(Nm)*0.02
    elif t > 0.700 and t < 0.720:
        return np.ones(Nm)*0.02
    elif t > 0.800 and t < 0.820:
        return np.ones(Nm)*0.02
    elif t > 0.900 and t < 0.920:
        return np.ones(Nm)*0.02
    elif t > 1.000 and t < 1.020:
        return np.ones(Nm)*0.02
    elif t > 1.100 and t < 1.120:
        return np.ones(Nm)*0.02
    elif t > 1.200 and t < 1.220:
        return np.ones(Nm)*0.02
    elif t > 1.300 and t < 1.320:
        return np.ones(Nm)*0.02
    elif t > 1.400 and t < 1.420:
        return np.ones(Nm)*0.02
    elif t > 1.500 and t < 1.520:
        return np.ones(Nm)*0.02
    elif t > 1.600 and t < 1.620:
        return np.ones(Nm)*0.02
    elif t > 1.700 and t < 1.720:
        return np.ones(Nm)*0.02
    else:
        return np.zeros(Nm)
    
# def maintain_func_second(t):
#     if t > 2.350 and t < 2.370:
#         return np.ones(Nm)*0.005
...
def get_data(inputs_first, inputs_second, inputs_reactivate, inputs_maintain_first, initialangles, n_trials, probelist, res):
    start = timer()
    ...
    #inputs maintain
    first_maint_start = range(500, 1800, 100)
    first_maint_stop = range(520, 1820, 100)
    for start_, stop_ in zip(first_maint_start, first_maint_stop):
        inputs_maintain_first[:,int(start_/res):int(stop_/res),:] = np.ones(Nm)*0.02

    # second_maint_start = range(2250, 4350, 100)
    # second_maint_stop = range(2270, 4370, 100)
    # for start_, stop_ in zip(second_maint_start, second_maint_stop):
    #     inputs_maintain_second[:,int(start_/res):int(stop_/res),:] = np.ones(Nm)*0.02
...
def create_model(seed=None):
    ...
    with model:
        ...
        if not batch_processing:
            ...
            model.maintain_first=nengo.Node(maintain_func_first,label='maintain_first')
        elif batch_processing:
            model.inputNode_first=nengo.Node(np.zeros(128*128))
            model.maintain_first=nengo.Node(np.zeros(Nm),size_out=Nm)
        ...
        if(True):
            ...
            if not batch_processing:
                ...
                # model.maintain_second=nengo.Node(maintain_func_second,size_out=Nm,label='maintain_second')
            elif batch_processing:
                model.inputNode_second=nengo.Node(np.zeros(128*128))
                model.reactivate=nengo.Node(np.zeros(Nm))
                # model.maintain_second=nengo.Node(np.zeros(Nm),size_out=Nm)
            ...
            # nengo.Connection(model.maintain_second, memory_second.neurons)
        ...
        nengo.Connection(model.maintain_first, memory_first.neurons)
...
def plot_sim_1(sp_1,sp_2,res_1,res_2,cal_1,cal_2, mem_1, mem_2):
    ...
    figure_name = "Sim_1/full_model/exp2_generic_reactivation_model/exp2_figure"
...
if nengo_gui_on:
    ...
    if sim_to_run == 2:
        ...
        input_data_maintain_first = np.zeros((minibatch_size, n_steps, Nm))
        # input_data_maintain_second = np.zeros((minibatch_size, n_steps, Nm))
        ...
        for subj in range(n_subj):
            ...
            for batch in range(n_batches):
                ...
                get_data(input_data_first, input_data_second, input_data_reactivate, input_data_maintain_first, data_initialangles, minibatch_size, probelist, res)
                ...
                sim.run_steps(
                    ...
                    data = {
                        ...
                        model.maintain_first: input_data_maintain_first, 
                        # model.maintain_second: input_data_maintain_second
                    },
                )
                ...
                if store_decisions:
                    for in_batch_trial in range(minibatch_size):
                        ...
                        np.savetxt(data_path+"Performance/full_model/exp2_generic_reactivation_model/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe1.csv" % (anglediff, subj+1, trial),
                            sim.data[model.p_dec_first][in_batch_trial,int((1800)/res):int((2100)/res),:], delimiter=",")

                        np.savetxt(data_path+"Performance/full_model/exp2_generic_reactivation_model/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe2.csv" % (anglediff, subj+1, trial),
                ...
                if store_memory:
                    ...
                    if (batch+1)%(n_batches/split)==0:
                        ...
                        np.save(data_path+"Decoding/full_model/exp2_generic_reactivation_model/subj_%i_mem_neuron_data_first_%i.npy" % (subj+1, split_index), neuro_mem_first)
                        np.save(data_path+"Decoding/full_model/exp2_generic_reactivation_model/subj_%i_mem_neuron_data_second_%i.npy" % (subj+1, split_index), neuro_mem_second)
                        np.save(data_path+"Decoding/full_model/exp2_generic_reactivation_model/subj_%i_initial_angles_%i.npy" % (subj+1, split_index), initialangles)
